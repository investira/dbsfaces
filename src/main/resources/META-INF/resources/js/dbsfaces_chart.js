			...
			for (var xI = 0; xI < pChartData.originalValues.length; xI++){
				...
				xLabels.forEach(function(pLabel, pRelationalGroupIndex){
					...
					if (xChartValueData == null){
						xChartValueData = dbsfaces.chartValue.initialize(pChartData, pLabel, pRelationalGroupIndex, xOriginalValue);
					...
					xChartValueData.displayValue = pChartData.valuePrefix;
					if (pChartData.valuePrefix.valueSufix == "" || pChartData.valuePrefix.valueSufix == "%"){
						xChartValueData.displayValue += dbsfaces.format.number(xChartValueData.value, pChartData.valueDecimalPlaces);
					}else{
						xChartValueData.displayValue += dbsfaces.format.numberSimplify(xChartValueData.value, pChartData.valueDecimalPlaces);
					}
					xChartValueData.displayValue += pChartData.valueSufix;
...
//			dbsfaces.chart.pvInitializeLayoutChartBar(pChartData);
	...
	pvInitializeLayoutChartValuesCreateDom: function(pChartData, pChartValueData){
		//Cria doms do chartValue
		dbsfaces.chartValue.initializeLayoutCreateDom(pChartData, pChartValueData);
		
		//Captura eventos do chartValue
